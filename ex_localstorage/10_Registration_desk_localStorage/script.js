...
$('.btn-result').bind('click', function (e){
	...
	if (resp1 != '' && resp2 != '' && resp3 != '' && resp4 != '' && resp5 != '') {
		...
		if (resp1 == 'Morning') {
			...
			$('.form-7 .check').append('<p> Primeira atividade - Você respondeu: <strong>' + resp1 + '. </strong><em>A Resposta está Errada!</em> A resposta correta seria Morning.</p>');			
		}		
		...
		if (resp2 == 'Schwartzbold') {
			...
			$('.form-7 .check').append('<p> Segunda atividade - Você respondeu: <strong>' + resp2 + '. </strong><em>A Resposta está Errada!</em> A resposta correta seria Schwartzbold.</p>');			
		}
		...
		if (resp3 == 'Adhelle') {
			...
			$('.form-7 .check').append('<p> Terceira atividade - Você respondeu: <strong>' + resp3 + '. </strong><em>A Resposta está Errada!</em> A resposta correta seria Adhelle</p>');			
		}
		...
		if (resp4 == 'Germany') {
			...
			$('.form-7 .check').append('<p> Quarta atividade - Você respondeu: <strong>' + resp4 + '. </strong><em>A Resposta está Errada!</em> A resposta correta seria Germany.</p>');			
		}

		if (resp5 == 'adhels@hitmail.de') {
			$('.form-7 .check').append('<p> Quarta atividade - Você respondeu: <strong>' + resp5 + '. </strong><em>A Resposta está Correta!</em></p>');
		}
		else if (resp5 == '') {
			$('.form-7 .check').append('<p> Quarta atividade - Você Não preencheu este campo!');
		}
		else {
			$('.form-7 .check').append('<p> Quarta atividade - Você respondeu: <strong>' + resp5 + '. </strong><em>A Resposta está Errada!</em> A resposta correta seria adhels@hitmail.de</p>');			
		}			
